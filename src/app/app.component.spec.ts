...
describe('AppComponent', () => {
    ...
    let restoreSession: Mock;

    beforeEach(async () => {
        restoreSession = vi.fn().mockName('restoreSession').mockReturnValue(of(true));

        const authStub: Pick<AuthService, 'isAuthenticated' | 'restoreSession' | 'logout'> = {
        ...
        vi.spyOn(TestBed.inject(Router), 'initialNavigation').mockReturnValue(undefined);
    ...
    it("n'affiche pas le splash si le backend répond immédiatement", async () => {
        ...
        await vi.advanceTimersByTimeAsync(SPLASH_DELAY_MS + SPLASH_FADE_MS);
        fixture.detectChanges();
        expect(splash()).toBeNull();
    });

    it('affiche le splash sur un 503 puis le retire après le 200', async () => {
        fixture.detectChanges();
        ...
        await vi.advanceTimersByTimeAsync(RETRY_DELAY_MS - SPLASH_DELAY_MS);
        httpMock.expectOne(READINESS_URL).flush(UP);
        await vi.advanceTimersByTimeAsync(0);
        fixture.detectChanges();
    ...
    it('réessaie après une erreur réseau sans afficher d’erreur définitive', async () => {
        fixture.detectChanges();
